...
export const signup = async (req, res) => {
    try {
        ...
        if (auth.currentUser) {
            await sendEmailVerification(auth.currentUser).catch((err) => console.log(`Error sending email verification: ${err}`));
        }
        // Update Firebase user profile to include name
        if (auth.currentUser) {
            await updateProfile(auth.currentUser, { displayName: name }).catch((err) => console.log(`Error updating profile: ${err}`));
        }
